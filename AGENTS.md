...
- **跨源请求一律 403**（第十一轮）：页面与 API 同源，默认不给任何 CORS 头；要跨源必须用 `config.json` 的 `cors_allow_origins` 显式点名来源（改完重启）。不要把它改回 `allow_origins=["*"]`。
- **`X-Forwarded-*` 只在同机代理（对端是回环）回源时才采信**（第十一轮）：判据是 `peer.is_loopback`，不是 `is_private`。服务默认 bind `0.0.0.0`，局域网直连的对端也是私网地址，采信它就等于让爆破防护的限流桶可被伪造的 XFF 轮换。
