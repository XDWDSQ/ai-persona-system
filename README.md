...
- 多角色人设对话：云端 OpenAI 兼容 API（小米 MiMo / DeepSeek / 火山方舟 / MiniMax / 自定义）；**本地 Qwen3.5-4B（llama.cpp / CUDA）已从界面下线**，代码保留，见文末「本地 LLM 说明」
- 角色引擎：按角色持久化记忆与状态，对话注入时间/位置/天气/状态/现实动态/记忆上下文，回复后异步更新；大帅支持主动问候
- 图片/文件附件：发送栏可发图片、文档和其他文件；图片走视觉链路（默认 `auto`：云端多模态优先，本地 Qwen2.5-VL 兜底 —— **本地引擎已下线，实际恒走云端**），让角色真正"看到"
- 联网搜索：模型判断知识过时时会自动搜索最新消息（默认 DuckDuckGo，Bing RSS 兜底），尤其适合查自己的比赛、战队与近况
- 语音合成与音色克隆：支持阿里云千问（qwen3-tts / qwen3-tts-vc 克隆音色）、MiniMax 海螺、小米 MiMo 三条云端链路（**本地 Qwen3-TTS 已从界面下线**，代码保留）；阿里云与 MiniMax 支持声音克隆，合成采样参数（语速/音量/音调/采样率）可在设置页调整
- 人设、语音参数可视化配置
...
| `cors_allow_origins` | **跨源白名单**（默认不配 = 不给任何跨源访问）。本项目的页面与 API 同源，正常使用无需配置；只有你另外写了个外部页面要调本服务时才把它的来源（如 `http://192.168.1.9:5173`）列进来。改完需重启服务（中间件在启动时注册）。 |
...
- **跨源一律拒绝**（第十一轮）：本项目的页面与 API **同源**（`chat.html` 由本服务自己发，所有请求都是相对路径），所以默认不给任何 CORS 头，`access_gate` 对带 `Origin` 且与本站不一致的请求直接 403。此前是 `allow_origins=["*"]`，而 `/api/login` 在口令校验**之前**——恶意网页可以跨源猜口令并凭 `Access-Control-Allow-Origin: *` 读懂 200/401，等于一个分布式口令 oracle（每个访客浏览器一个出口 IP，按 IP 的锁定跟不上）。确实需要跨源时在 `config.json` 配 `cors_allow_origins` 显式点名来源。
- **X-Forwarded-For 只在同机代理（回环）回源时采信**（第十一轮）：服务默认 bind `0.0.0.0`，局域网里任何机器直连过来对端都是私网地址；旧实现把"私网"当自己人，于是对方每请求换一个伪造 `X-Forwarded-For` 就换一个限流桶，"8 次失败锁 10 分钟"变成可无限重试。现在判据只看对端是否回环（ngrok 不受影响）。
- **用户自填 API 地址有 SSRF 闸**（第九轮起，第十一轮收紧）：`/api/llm-models`、`/api/llm-quota` 会拒绝把服务端指向回环/链路本地/云元数据地址，含 `2130706433`、`0x7f000001`、`127.1`、`[::ffff:127.0.0.1]`、`localhost.` 等全部等价写法，并对域名做解析后复查（挡 `127.0.0.1.nip.io` 这类"公网域名解析到本机"）。局域网地址（如自建 ollama 的 `192.168.x.x`）**仍然放行**。
- **现在只在本机跑**（2026-09 起不再做远程/公网部署）：直接访问 `http://127.0.0.1:8000`；`access_token` 因此变成**可选的本地保护**（配了就生效，不配也不可怕）。
- 服务仍绑定 `0.0.0.0`（历史配置，方便局域网直连）。若只在本机用、想让局域网也进不来，把启动脚本里的 host 改成 `127.0.0.1` 即可。

## 手机远程访问（ngrok）—— 已退役，保留备查

> **2026-09 起不再做远程部署**：改为本机开发、本机启动，服务地址就是 `http://127.0.0.1:8000`；
> `data/tunnel_url.txt` 不再维护，也不用再启动隧道。以下流程保留备查；真要走隧道时，
> **务必先配好 `access_token`**再启动 —— 公网暴露等于把全部页面与 API 对全互联网开放。

电脑跑本地服务、手机随时随地访问（ngrok 免费版）：
