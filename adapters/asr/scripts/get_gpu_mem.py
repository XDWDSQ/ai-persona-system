...
def _read_nvidia_smi_memory() -> dict[str, float] | None:
    """NVIDIA 独显：用 nvidia-smi 精确读取显存（单位 GB）。"""
    ...
    try:
        ...
        total = float(total_mb) / 1024.0
        used = float(used_mb) / 1024.0
        return {"gpu_mem_total": total, "gpu_mem_free": max(total - used, 0.0)}
    except Exception:
        return None
...
def get_gpu_memory_win() -> dict[str, float]:
    ...
    # 优先 NVIDIA：nvidia-smi 直接给精确占用（本机 RTX 4060）。
    nv = _read_nvidia_smi_memory()
    if nv is not None:
        return {**base, **nv}

    # 回退 DXGI：任意硬件 GPU（Intel/NVIDIA/AMD），不再限定 Intel。
    gpu_infos = _collect_gpu_memory_info(include_software=False)
    adapter = next(
        (g for g in gpu_infos if g["vendor_id"] != 0 and not g["is_software_adapter"]),
        None,
    )
    if adapter is None:
        raise RuntimeError("未找到可用的硬件 GPU")

    gpu_mem_total = int(adapter["gpu_memory_total"])
    gpu_mem_used = adapter["gpu_memory_used"]
    if gpu_mem_used is None:
        # 占用读取失败时不再抛错：按"显存充足"处理，避免 server-dog
        # 把刚拉起的服务当内存不足反复驱逐（Intel-only 的老 bug）。
        gpu_mem_free = gpu_mem_total
    else:
        gpu_mem_free = max(gpu_mem_total - int(gpu_mem_used), 0)
    return {
        **base,
        "gpu_mem_total": _bytes_to_gb(gpu_mem_total),
        "gpu_mem_free": _bytes_to_gb(gpu_mem_free),
    }
