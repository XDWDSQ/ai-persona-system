...
def _pick_device(log_path: Path | None) -> str:
    # 适配副本：默认 CPU（Iris Xe 核显对 fp16 ASR 部分算子无内核会崩溃），
    # 可用环境变量 LOCAL_ASR_DEVICE 强制指定（如 GPU.0）
    import os
    import openvino as ov
    forced = os.environ.get("LOCAL_ASR_DEVICE", "CPU")
    devices = ov.Core().available_devices
    _log_message(log_path, f"openvino devices: {devices}, forced: {forced}")
