...
聊天页发送栏新增了「发送文件」按钮（图片/文档/文件），图片会优先送到
`http://127.0.0.1:11435/v1` 的视觉模型，让角色真正看到图；视觉服务未启动时
自动退回普通文本模型，仅附带文件名提示。可在 `config.json` 的 `vision` 节点
修改地址和模型（默认 `Qwen2.5-VL-3B`）。
